...
class Genome:
    def __init__(self,input,output,baby=False):
        self.score = 0
        self.input = input
        self.output = output
        self.layers = {"Inputs":Layer(self,"Inputs",0,self.input),"Hiddens":[Layer(self,"Hiddens",0,int(self.input/3))],
        "Outputs":Layer(self,"Outputs",0,self.output)}
        if not baby:
            self.mutate(10)

    def mutate(self,repeat=1):
    ...
    def generate_baby(self,parent,a=0):
        ...
        baby = Genome(self.input,self.output,True)
        baby.layers["Hiddens"] = []

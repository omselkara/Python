...
class ödül_pencere:
    def __init__(self,parent):
        ...
        "Kazanılmış Hak Aylıkta Değerlendirilen Hizmet Süresi","Üstün Başarı Belgesi Tarih Ve Sayısı","İl","İlçe","Ödüle Esas Puanı","Yıl İçerisinde Kınama Cezasından Ağır Ceza Almışmı",
                        "Disiplin Amirinin Adı","Disiplin Amirinin Unvanı","(BOŞ)","(BOŞ)"]
        self.entryler = []
...
hizmetin yürütülmesinde dikkate değer çalışmalar yapmak (tescil edilmiş olmalı)"""]
    ...
    def yazdır(self):
        if self.seçme.current()!=0:
            ...
            ws["A51"].value = str(kişi.bilgiler[11])
            ws["A52"].value = str(kişi.bilgiler[12])
